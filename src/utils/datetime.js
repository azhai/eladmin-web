...
Date.prototype.daysAgo = function (days, midnight) {
  days = days ? days - 0 : 0;
  let date = new Date(this.getTime() - days * 8.64E7);
...
Date.prototype.quarterBegin = function () {
  let month = this.getMonth() - this.getMonth() % 3;
...
Date.prototype.strftime = function (format, local) {
  ...
  return format.replace(/%[A-Za-z%]/g, function (f) {
    ...
    switch (f) {
      ...
      case "%U":
      case "%W":
        let ws = _this.getWeeksOfYear("%W" === f);
        ans = padZero(ws, 2);
        break;
      ...
      case "%z":
        ...
        let sign = tzo < 0 ? "-" : "+";
        tzo = Math.abs(tzo);
        let ho = padZero(tzo / 60, 2);
        let mo = padZero(tzo % 60, 2);
        ans = sign + ho + mo;
        break;
...
Date.prototype.humanize = function (local) {
  local = local && local.startsWith("zh") ? "zh" : "en";
  let result = this.strftime("", local);
  let days = (Date.today() - this.toMidnight().getTime()) / 8.64E7;
  if (days <= -10 || days >= 10) {
    return result;
  }
  let labels = local_labels.dayagos[local];
  let lbl = "";
  if (days === 0 || days === 1) {
    lbl = labels[days];
  } else if (days === -1) {
